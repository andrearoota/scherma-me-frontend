...
export default function TableBase ({ data, isError, isLoading }: TableProps): JSX.Element {
  ...
  const columns = useMemo<Array<MRT_ColumnDef<any>>>(
    () => [
      ...
      {
        header: 'Punti per sesso',
        id: 'gender',
        columns: [
          {
            header: 'Femminile',
            accessorKey: 'genderByPoints.femminile',
            Cell: (row) => formatNumber(row.cell.getValue<number>())
          },
          {
            header: 'Maschile',
            accessorKey: 'genderByPoints.maschile',
            Cell: (row) => formatNumber(row.cell.getValue<number>())
          }
        ]
      },
      {
        header: 'Punti per arma',
        id: 'weapon',
        columns: [
          {
            header: 'Fioretto',
            accessorKey: 'weaponByPoints.fioretto',
            Cell: (row) => formatNumber(row.cell.getValue<number>())
          },
          {
            header: 'Sciabola',
            accessorKey: 'weaponByPoints.sciabola',
            Cell: (row) => formatNumber(row.cell.getValue<number>())
          },
          {
            header: 'Spada',
            accessorKey: 'weaponByPoints.spada',
            Cell: (row) => formatNumber(row.cell.getValue<number>())
          }
        ]
      },
